...
public class UdpSend {
    ...
    public void online() {
        Messages msg = new Messages(MyApp.mac,DateTools.getTime(), Tools.getName(), MessageType.ONLINE, null);
        Send send = new Send(msg, false, MultiCastAddress);
        send.start();
    }

    public void onlineReply(String ip) {
        Messages msg = new Messages(MyApp.mac, DateTools.getTime(), Tools.getName(), MessageType.ONLINE_REPLY, null);
        Send send = new Send(msg, false, ip);
        send.start();
    }

    public void sendMessage(String ip,long time, String message) {
        Messages msg = new Messages(MyApp.mac, time, Tools.getName(), MessageType.NORMAL_MESSAGE, message);
        Send send = new Send(msg, false, ip);
        send.start();
    }

    public void sendFileRequest(String ip, Serializable files) {
        Messages msg = new Messages(MyApp.mac, DateTools.getTime(), Tools.getName(), MessageType.FileSendRequest, files);
        Send send = new Send(msg, false, ip);
        send.start();
    }

    public void sendFileReply(String ip,Serializable filePath) {
        Messages msg = new Messages(MyApp.mac, DateTools.getTime(), Tools.getName(), MessageType.FileSendReply, filePath);
        Send send = new Send(msg, false, ip);
        send.start();
    }
